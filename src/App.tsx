  ...
  const processUploadedPdf = async (file: File, expId: string) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!pdfjsLib) {
          throw new Error("PDF.js library is still loading. Please wait a moment and try again.");
        }

        // Configure Worker - Use unpkg.com as backup CDN
        try {
          pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.worker.min.js';
        } catch (err) {
          console.warn("Could not set worker from unpkg, trying cdnjs...");
        ...
        if (err.message && err.message.includes("loading")) {
          errorMessage = "PDF.js is still loading. Please wait 3 seconds and try again.";
          ...
          errorMessage += "Please verify this is a valid PDF file.";
        }
    ...
    reader.onerror = () => {
      setPdfError("Error reading the file. Please try again with a different PDF.");
      setIsPdfLoading(false);
    };
          ...
          >
                ...
                <div className="border border-gold-dim/20 bg-card/60 rounded-[1px] min-h-[320px] flex flex-col items-center justify-center p-6 relative overflow-hidden">
                  ...
                  {isPdfLoading ? (
                    ...
                    <div className="w-full text-center py-8">
                      ...
                      {/* Certificate Image Display - Automatically Shown */}
                      {activeModalExp.certificateImage ? (
                        <div className="max-w-4xl mx-auto mt-8">
                          <div className="mb-4 flex items-center justify-center gap-2">
                            <ShieldCheck className="w-5 h-5 text-gold-primary" />
                            <h4 className="font-bebas text-[13px] tracking-[0.2em] text-gold-bright">
                              ORIGINAL CERTIFICATE SCAN
                            </h4>
                            <ShieldCheck className="w-5 h-5 text-gold-primary" />
                          </div>
                          <div className="border-2 border-gold-primary/30 bg-void/40 p-4 rounded-lg shadow-2xl">
                            <img 
                              src={activeModalExp.certificateImage}
                              alt={`Certificate for ${activeModalExp.company.en}`}
                              className="w-full h-auto rounded shadow-lg"
                              loading="lazy"
                            />
                            <div className="mt-4 text-center">
                              <p className="font-bebas text-[10px] tracking-[0.15em] text-gold-dim/80">
                                ✓ AUTHENTICATED ORIGINAL DOCUMENT
                              </p>
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="max-w-md mx-auto mt-8 border border-dashed border-gold-dim/20 bg-void/40 p-6 rounded text-center">
                          <FileText className="w-8 h-8 text-gold-dim/50 mx-auto mb-2" />
                          <p className="text-[11px] text-lux-gray/70">
                            Certificate scan not yet available for this position
                          </p>
                        </div>
                      )}
            ...
            <div className="p-6 bg-surface border-t border-faint text-center flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
              <span className="font-bebas text-[9px] text-[#8a8070] tracking-[0.2em] uppercase select-none">
                🔏 OFFICIAL STAMPS VISIBLE ON THE ORIGINAL PHYSICAL DOCUMENTS
              </span>
              <button 
                onClick={() => setActiveModalExp(null)}
